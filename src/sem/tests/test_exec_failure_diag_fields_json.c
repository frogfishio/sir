...
int main(void) {
  char sir_path[] = "/tmp/sem_exec_failure_diag_json_XXXXXX";
  ...
  // Call zi_write with a null/invalid pointer so the hosted runtime returns ZI_E_BOUNDS.
  if (!write_all(out, "{\"ir\":\"sir-v1.0\",\"k\":\"meta\",\"producer\":\"sem-unit\",\"unit\":\"bad_exec\"}\n")) return fail("write failed");
  ...
  if (!write_all(out, "{\"ir\":\"sir-v1.0\",\"k\":\"node\",\"id\":110,\"tag\":\"const.i32\",\"type_ref\":1,\"fields\":{\"value\":1}}\n"))
    return fail("write failed");
  if (!write_all(out, "{\"ir\":\"sir-v1.0\",\"k\":\"node\",\"id\":111,\"tag\":\"const.i32\",\"type_ref\":1,\"fields\":{\"value\":1}}\n"))
    return fail("write failed");
  if (!write_all(out, "{\"ir\":\"sir-v1.0\",\"k\":\"node\",\"id\":112,\"tag\":\"const.i64\",\"type_ref\":2,\"fields\":{\"value\":0}}\n"))
    return fail("write failed");

  if (!write_all(out,
                 "{\"ir\":\"sir-v1.0\",\"k\":\"node\",\"id\":120,\"tag\":\"call.indirect\",\"type_ref\":1,\"fields\":{\"sig\":{\"t\":\"ref\",\"id\":10},\"args\":[{\"t\":\"ref\",\"id\":100},{\"t\":\"ref\",\"id\":110},{\"t\":\"ref\",\"id\":112},{\"t\":\"ref\",\"id\":111}]}}\n"))
    return fail("write failed");
  ...
  char diag_path[] = "/tmp/sem_exec_failure_diag_json_out_XXXXXX";
  ...
  int prog_rc = 0;
  const int rc = sem_run_sir_jsonl_capture_ex(sir_path, NULL, 0, NULL, SEM_DIAG_JSON, false, &prog_rc);
  ...
  if (rc == 0) {
    unlink(diag_path);
    return fail("expected tool failure (exec error)");
  }
  ...
  if (strstr(line, "\"code\":\"sem.exec\"") == NULL) return fail("expected sem.exec diagnostic");
  if (strstr(line, "\"fid\":") == NULL) return fail("missing fid field in JSON exec diagnostic");
  if (strstr(line, "\"ip\":") == NULL) return fail("missing ip field in JSON exec diagnostic");
  if (strstr(line, "\"op\":\"call.extern\"") == NULL) return fail("missing op field in JSON exec diagnostic");
  if (strstr(line, "\"node\":120") == NULL) return fail("missing node field in JSON exec diagnostic");
  if (strstr(line, "\"line\":") == NULL) return fail("missing line field in JSON exec diagnostic");
  return 0;
}
