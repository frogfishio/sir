...
static int32_t fd_read(void* ctx, sem_guest_mem_t* mem, zi_ptr_t dst_ptr, zi_size32_t cap) {
  sir_fd_stream* s = (sir_fd_stream*)ctx;
...
static int open_under_root(const char* root, const uint8_t* guest_path, uint32_t guest_len, int flags, mode_t mode, int* out_fd) {
...
zi_handle_t sir_hosted_file_fs_open_from_params(sir_hosted_file_fs_t* fs, sem_handles_t* hs, sem_guest_mem_t* mem, zi_ptr_t params_ptr,
  ...
  if (!sem_guest_mem_map_ro(mem, path_ptr, (zi_size32_t)path_len, &path_bytes) || !path_bytes) return (zi_handle_t)ZI_E_BOUNDS;
  if (has_embedded_nul(path_bytes, path_len)) return (zi_handle_t)ZI_E_INVALID;
  ...
  if ((of & (ZI_FILE_O_TRUNC | ZI_FILE_O_APPEND)) && !want_w) return (zi_handle_t)ZI_E_INVALID;
  ...
  if (rr != 1) return (zi_handle_t)((rr == 0) ? ZI_E_DENIED : rr);

  sir_fd_stream* s = (sir_fd_stream*)calloc(1, sizeof(*s));
  if (!s) {
    (void)close(fd);
